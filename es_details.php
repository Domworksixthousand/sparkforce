...
if ($rent_id) {
    ...
    if ($result_execute_house->num_rows > 0) {
        while ($row_h = $result_execute_house->fetch_assoc()) {
            $status = $row_h['status'] ?? 'Available';
            if (strcasecmp($status, 'Available') === 0) {
                $availableEventSpaces++;
            }
            $event_units[] = [
                'es_id'  => $row_h['es_id'],
                'type'   => $row_h['type'],
...
$totalEventSpaces = count($event_units);
...
<div class="drawer lg:drawer-open">
  ...
  <div class="drawer-content flex flex-col">
    ...
    <header class="flex items-center py-3 px-4 bg-base-100 shadow-sm lg:hidden sticky top-0 z-30">
      <div class="flex-none">
      ...
      <div class="flex-1 px-2">
        <span class="font-bold text-lg truncate block"><?= htmlspecialchars($property_name ?: 'Event Space Details'); ?></span>
      </div>
    </header>
            ...
            <div class="relative h-64 sm:h-80 md:h-[360px] w-full rounded-xl overflow-hidden bg-base-300 group shadow-inner">
              ...
              <div class="absolute top-4 left-4">
                <span class="badge badge-success text-white badge-lg gap-2 shadow-md">
                  <i class="fa-solid fa-calendar-day"></i> Event Space
                </span>
              </div>
            ...
            <div>
              ...
              <div class="grid grid-cols-2 gap-3 mt-4">
                <div class="stat bg-base-200/60 p-3 rounded-xl">
                  <div class="stat-title text-xs">Total Venues/Spaces</div>
                  <div class="stat-value text-xl"><?= $totalEventSpaces; ?></div>
                </div>
                <div class="stat bg-base-200/60 p-3 rounded-xl">
                  <div class="stat-title text-xs">Available</div>
                  <div class="stat-value text-xl text-success"><?= $availableEventSpaces; ?></div>
                </div>
              </div>
            </div>

            <div class="p-4 bg-base-200/50 rounded-xl border border-base-300 space-y-2">
              <p class="text-xs font-bold text-base-content/60 uppercase tracking-wider">Host Contact</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-300">
            <h2 class="text-lg font-bold mb-4 flex items-center gap-2">
              <i class="fa-solid fa-square-check text-success"></i> Venue Features & Amenities
            </h2>
            ...
            <?php else: ?>
              <p class="text-sm text-base-content/50 italic">No specific venue amenities listed.</p>
            <?php endif; ?>
          </div>

          <!-- Event Spaces Breakdown -->
          <div class="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-300">
            <div class="flex items-center justify-between mb-4">
              <h2 class="text-lg font-bold flex items-center gap-2">
                <i class="fa-solid fa-champagne-glasses text-success"></i> Available Event Halls & Spaces
              </h2>
              <span class="text-xs text-base-content/60"><?= $totalEventSpaces; ?> Total</span>
            </div>

            <?php if (!empty($event_units)): ?>
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <?php foreach ($event_units as $unit): ?>
                  <div class="border border-base-300 rounded-xl p-4 flex flex-col justify-between bg-base-100 hover:border-primary transition-all shadow-xs gap-3">

                    <div class="flex items-center justify-between gap-2">
                      <h3 class="font-bold text-base truncate">Space #<?= htmlspecialchars($unit['es_id']); ?></h3>
                      <span class="badge badge-sm <?= eventStatusBadge($unit['status']); ?>">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-xs text-base-content/80 bg-base-200/50 p-2.5 rounded-lg border border-base-200">
                      <?php if (!empty($unit['type'])): ?>
                        <div class="flex items-center gap-1.5 col-span-2" title="Venue Type">
                          <i class="fa-solid fa-masks-theater text-success text-[11px]"></i>
                          <span><strong>Type:</strong> <?= htmlspecialchars(ucfirst($unit['type'])); ?></span>
                        </div>
                      <?php endif; ?>

                      <?php if (!empty($unit['area'])): ?>
                        <div class="flex items-center gap-1.5 col-span-2" title="Floor Area / Capacity">
                          <i class="fa-solid fa-ruler-combined text-success text-[11px]"></i>
                          <span><strong>Area:</strong> <?= htmlspecialchars($unit['area']); ?> sqm</span>
                        </div>
                      <?php endif; ?>
                    </div>
                    ...
                    <div class="pt-1 border-t border-base-200 flex items-center justify-between">
                      ...
                      <p class="text-xs font-semibold text-success">
                        ₱<?= number_format((float)$unit['price'], 2); ?>
                      </p>
                    </div>
            ...
            <?php else: ?>
              <div class="text-center py-8 text-base-content/50">
                ...
                No individual event spaces/halls listed under this venue yet.
              </div>
            <?php endif; ?>
          </div>
        ...
        <div class="space-y-6">
          <div class="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-300 sticky top-6">
            ...
            <div id="map" class="w-full h-72 rounded-xl border border-base-300 z-0"></div>
          </div>
        </div>
      ...
      <div class="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-300">
        <div class="flex justify-between mb-6 pb-3 border-b border-base-200 flex-col lg:flex-row">
          <div class="mb-3">
            <h2 class="text-xl font-bold flex items-center gap-2 text-base-content">
              <i class="fa-solid fa-images text-success"></i> Event Space Photos
            </h2>
            <p class="text-xs text-base-content/60 mt-0.5">Explore photos of the event venue and layout</p>
          </div>
        ...
        <?php else: ?>
          <div class="text-center py-10 text-base-content/50 bg-base-200/50 rounded-xl">
            ...
            <p class="text-sm font-medium">No gallery images uploaded for this event space.</p>
          </div>
        <?php endif; ?>
      </div>
        ...
        <ul class="menu p-0 w-full gap-1">
          ...
          if ($landlord_id) {
              ...
              if ($result_other->num_rows > 0) {
                      ...
                      <li>
                        <a href="?id=<?= urlencode($other_id); ?>" class="flex items-center gap-3 p-2.5 rounded-xl transition-all <?= $isActive; ?>">
                          ...
                          <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs opacity-75 mt-0.5">₱<?= number_format((float)$other_price, 2); ?></p>
                          </div>
                        </a>
                      </li>
                      <?php
                  ...
                  echo '<li class="p-4 text-center text-xs text-base-content/50 italic bg-base-200/50 rounded-xl mt-2">No other event venues found.</li>';
              }
          }
          ?>
        </ul>
