  ...
  <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden transform transition-all">
    ...
    <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/70 sticky top-0 z-10 backdrop-blur-md">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h2 class="text-lg font-bold text-slate-800">House Configuration</h2>
          <p class="text-xs text-slate-500">Manage house details, pricing, photos, unit specs, and amenities.</p>
        </div>
      </div>
    ...
    <form action="../functions.php" method="POST" enctype="multipart/form-data" class="p-6 overflow-y-auto space-y-8" id="mainForm">
        ...
        <div class="space-y-4">
            <div class="border-b border-slate-100 pb-2">
                <h3 class="text-sm font-bold uppercase tracking-wider text-slate-400">House Information</h3>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="space-y-1.5">
                    <label class="block text-xs font-semibold text-slate-600">House Name / Number *</label>
                    <div class="relative flex items-center">
                        ...
                        <input type="text"
                               ...
                               name="house_name"
                               value="<?php echo htmlspecialchars($_SESSION['house_name'] ?? ''); ?>"
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative flex items-center">
                        ...
                        <input type="text"
                               ...
                               name="house_price"
                               value="<?php echo htmlspecialchars($_SESSION['house_price'] ?? ''); ?>"
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative flex items-center">
                        <input type="text"
                               ...
                               name="house_rate"
                               value="<?php echo htmlspecialchars($_SESSION['house_rate'] ?? ''); ?>"
            ...
            <div class="space-y-1.5 pt-2" id="cover-section">
                ...
                <?php if (!empty($_SESSION['house_cover'])): ?>
                    <input type="hidden" name="old_cover" value="<?php echo htmlspecialchars($_SESSION['house_cover']); ?>">
                    <div class="flex items-center justify-between bg-emerald-50/60 border border-emerald-100 p-2.5 rounded-xl text-xs mb-2">
                        <div class="flex items-center gap-3">
                            <img src="../assets/uploads/<?php echo htmlspecialchars($_SESSION['house_cover']); ?>"
                                 class="w-10 h-10 object-cover rounded-lg border border-white shadow-xs"
                                 alt="Preview">
                            <span class="text-emerald-900">Previously selected:
                                <strong class="underline font-medium"><?php echo htmlspecialchars($_SESSION['house_cover']); ?></strong>
                            </span>
                        </div>
                ...
                <label id="cover-label" class="flex flex-col items-center justify-center w-full h-24 border-2 border-slate-200 border-dashed rounded-xl cursor-pointer bg-slate-50/50 hover:bg-slate-50 hover:border-emerald-400 transition-all">
                    ...
                    <input type="file"
                           ...
                           <?php echo !empty($_SESSION['house_cover']) ? '' : 'required'; ?> />
                </label>
            ...
            <div class="space-y-1.5 pt-2">
                ...
                <div class="border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all">
                    <textarea
                        id="myEditor"
                        name="apartment_other_info"><?php echo htmlspecialchars($_SESSION['apartment_other_info'] ?? ''); ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label class="block text-xs font-semibold text-slate-600">House Type *</label>
                    <div class="relative flex items-center">
                        <span class="absolute left-3.5 text-slate-400 pointer-events-none">
                        ...
                        <select class="w-full pl-10 pr-3 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all" name="type" required>
                            <option value="" disabled <?php echo empty($_SESSION['type']) ? 'selected' : ''; ?>>Select House Type</option>
                            <option value="Single-Family" <?php echo (($_SESSION['type'] ?? '') == 'Single-Family') ? 'selected' : ''; ?>>Single-Family</option>
                            <option value="Townhouse" <?php echo (($_SESSION['type'] ?? '') == 'Townhouse') ? 'selected' : ''; ?>>Townhouse</option>
                            <option value="Duplex" <?php echo (($_SESSION['type'] ?? '') == 'Duplex') ? 'selected' : ''; ?>>Duplex</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-semibold text-slate-600">Bedrooms *</label>
                    <div class="relative flex items-center">
                        <span class="absolute left-3.5 text-slate-400 pointer-events-none">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 4v16"/><path d="M2 8h18a2 2 0 0 1 2 2v10"/><path d="M2 17h20"/><path d="M6 8v9"/></svg>
                        </span>
                        <select class="w-full pl-10 pr-3 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all" name="bedroom" required>
                            <option value="" disabled <?php echo empty($_SESSION['bedroom']) ? 'selected' : ''; ?>>Select Bedroom</option>
                            <option value="Studio" <?php echo (($_SESSION['bedroom'] ?? '') == 'Studio') ? 'selected' : ''; ?>>Studio</option>
                            <option value="1 Bed Room" <?php echo (($_SESSION['bedroom'] ?? '') == '1 Bed Room') ? 'selected' : ''; ?>>1 Bed Room</option>
                            <option value="2 Bed Room" <?php echo (($_SESSION['bedroom'] ?? '') == '2 Bed Room') ? 'selected' : ''; ?>>2 Bed Room</option>
                            <option value="3 Bed Room" <?php echo (($_SESSION['bedroom'] ?? '') == '3 Bed Room') ? 'selected' : ''; ?>>3 Bed Room</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-semibold text-slate-600">Parking / Outdoor *</label>
                    <div class="relative flex items-center">
                        <span class="absolute left-3.5 text-slate-400 pointer-events-none">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M9 17V7h4a3 3 0 0 1 0 6H9"/></svg>
                        </span>
                        <select class="w-full pl-10 pr-3 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all" name="parking" required>
                            <option value="" disabled <?php echo empty($_SESSION['parking']) ? 'selected' : ''; ?>>Select Parking / Outdoor</option>
                            <option value="Garage" <?php echo (($_SESSION['parking'] ?? '') == 'Garage') ? 'selected' : ''; ?>>Garage</option>
                            <option value="Private Yard" <?php echo (($_SESSION['parking'] ?? '') == 'Private Yard') ? 'selected' : ''; ?>>Private Yard</option>
                            <option value="Balcony" <?php echo (($_SESSION['parking'] ?? '') == 'Balcony') ? 'selected' : ''; ?>>Balcony</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3 sticky bottom-0 bg-white py-3 -mx-6 px-6">
            ...
            <button type="submit" name="save_house"
...
tinymce.init({
    ...
    init_instance_callback: function (editor) {
        const content = <?php echo json_encode($_SESSION['apartment_other_info'] ?? ''); ?>;
        if (content) {
