...
<dialog id="my_modal_3" class="modal" open>
  <div class="modal-box w-11/12 max-w-1xl ">
    ...
    <form action="../functions.php" method="POST" enctype="multipart/form-data">
        ...
        <p class="mb-3 font-bold">Commercial Space Information</p>
        ...
        <div class="w-[100%] flex flex-col gap-3 mb-5">
            <span class="w-[100%]">
                ...
                <label class="input w-[100%] flex items-center gap-2">
                    ...
                    <input type="text" class="autoInput grow w-[100%]" name="cs_name" value="<?php echo htmlspecialchars($_SESSION['cs_name'] ?? ''); ?>" placeholder="Enter Name / Number" required />
                </label>
            </span>
            ...
            <span class="w-[100%]">
                ...
                <label class="input w-[100%] flex items-center gap-2">
                    ...
                    <input type="text" class="numbers_only grow w-[100%]" name="cs_price" value="<?php echo htmlspecialchars($_SESSION['cs_price'] ?? ''); ?>" placeholder="Enter Price /Month" required />
                </label>
            </span>
            <span class="w-[100%]">
                ...
                <label class="input w-[100%]">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-5 text-gray-500"><path d="M20 11H4"/><path d="M20 7H4"/><path d="M7 21V4a1 1 0 0 1 1-1h4a1 1 0 0 1 0 12H7"/></svg>
                    <input type="text" class="autoInput grow w-[100%]" name="cs_rate"  value="<?php echo $_SESSION['cs_rate'] ?? ''; ?>" placeholder="Enter Rate" required />
                </label>
            </span>

            <span class="w-[100%]">
                <p class="mb-2 text-sm"> Cover Photo * </p>
                <?php if (!empty($_SESSION['cs_cover'])): ?>
                    <input type="hidden" name="old_cover" value="<?php echo htmlspecialchars($_SESSION['cs_cover']); ?>">
                    <div class="alert alert-success bg-success/10 text-success border border-success/20 p-2 mb-2 text-xs flex items-center justify-between rounded-lg">
                        <div class="flex items-center gap-2">
                            <img src="../assets/uploads/<?php echo htmlspecialchars($_SESSION['cs_cover']); ?>" class="w-10 h-10 object-cover rounded" alt="Preview">
                            <span>Previously selected: <strong class="underline"><?php echo htmlspecialchars($_SESSION['cs_cover']); ?></strong></span>
                        </div>
                ...
                <label class="input w-[100%] flex items-center gap-2">
                    ...
                    <input 
                        ...
                        <?php echo !empty($_SESSION['cs_cover']) ? '' : 'required'; ?> 
                    />
                </label>
            </span>
        </div>
            ...
            <div class="w-[100%] mb-3">
                ...
                <label class="input w-[100%] flex items-center gap-2">
                    ...
                    <select class="select w-[100%]" name="type" required>
                        <option value="" disabled <?php echo empty($_SESSION['type']) ? 'selected' : ''; ?>>Select Commercial Type</option>
                        <option value="Office Space" <?php echo (($_SESSION['type'] ?? '') == 'Office Space') ? 'selected' : ''; ?>>Office Space</option>
                        <option value="Retail Space" <?php echo (($_SESSION['type'] ?? '') == 'Retail Space') ? 'selected' : ''; ?>>Retail Space</option>
                        <option value="Industrial & Warehouse Space" <?php echo (($_SESSION['type'] ?? '') == 'Industrial & Warehouse Space') ? 'selected' : ''; ?>>Industrial & Warehouse Space</option>
                        <option value="Hospitality & Food Service" <?php echo (($_SESSION['type'] ?? '') == 'Hospitality & Food Service') ? 'selected' : ''; ?>>Hospitality & Food Service</option>
                        <option value="Multifamily (Commercial Classification)" <?php echo (($_SESSION['type'] ?? '') == 'Multifamily (Commercial Classification)') ? 'selected' : ''; ?>>Multifamily (Commercial Classification)</option>
                        <option value="Special Purpose & Miscellaneous" <?php echo (($_SESSION['type'] ?? '') == 'Special Purpose & Miscellaneous') ? 'selected' : ''; ?>>Special Purpose & Miscellaneous</option>
                    </select>
                </label>
            </div>
        ...
        <div id="amenities-container2">
            ...
            if (empty($_SESSION['amenities']) || !is_array($_SESSION['amenities'])) {
                $_SESSION['amenities'] = [""];
            }
            ...
            <div class="amen-item flex items-center gap-2 border border-gray-300 rounded-md p-1 mb-2">
                ...
                <select class="select w-[100%]" name="cs_amenity[]" required>
                    ...
                    <?php foreach ($amenities as $amen) { ?>
                        <option value="<?= htmlspecialchars($amen['amen_id']); ?>"
        ...
        <div class="text-end mt-4">
            <button type="submit" name="save_cs" class="btn btn-success text-white">Save</button>
        </div>
    </form>
  </div>
</dialog>
