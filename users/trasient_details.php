...
$rate = "";
$transient_id = $square_area = $type_transient = $status = "";
$apartment_images = [];
$saved_amenities = [];
...
if ($rent_id) {
    ...
    if ($result_transient->num_rows > 0) {
        ...
        $status         = $row_transient['status']       ?? 'Available';
    }
            ...
            <div class="relative h-64 sm:h-80 md:h-[360px] w-full rounded-xl overflow-hidden bg-base-300 group shadow-inner">
              ...
              <div class="absolute top-4 left-4">
                <span class="badge badge-success text-white badge-lg gap-2 shadow-md">
                  <i class="fa-solid fa-bed"></i> <?= htmlspecialchars(ucfirst($type_transient ?: 'Transient Space')); ?>
                </span>
              </div>
              ...
              <form action="../functions.php" method="POST" class="tooltip tooltip-left absolute bottom-4 right-4 h-fit lg:top-4" data-tip="<?= ($switch == 'on') ? 'Remove from Favorites' : 'Add to my Favorites'; ?>">
                ...
                <input type="hidden" name="locate" value="transient_details.php">
              ...
              <div class="grid grid-cols-2 gap-3 mt-4">
                ...
                <div class="stat bg-base-200/60 p-3 rounded-xl">
                  ...
                  <div class="stat-value text-xl">
                    <span class="badge <?= transientStatusBadge($status); ?>"><?= htmlspecialchars($status !== '' ? $status : 'Unknown'); ?></span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-300">
            <h2 class="text-lg font-bold mb-3 flex items-center gap-2">
              <i class="fa-solid fa-circle-info text-success"></i> About This Transient Space
            </h2>
            <p class="text-base-content/80 text-sm leading-relaxed whitespace-pre-line">
              <?= $other_info ? $other_info : 'No description available for this transient space.'; ?>
            </p>
          </div>
          ...
          <div class="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-300">
            <div class="flex items-center justify-between mb-4">
              <h2 class="text-lg font-bold flex items-center gap-2">
                <i class="fa-solid fa-bed text-success"></i> Transient Unit Details
              </h2>
              <span class="text-xs text-base-content/60">Unit ID: <?= htmlspecialchars($transient_id ?: 'N/A'); ?></span>
            </div>
            ...
            <?php if (!empty($transient_id)): ?>
              <div class="border border-base-300 rounded-xl p-4 flex flex-col justify-between bg-base-100 hover:border-primary transition-all shadow-2xs gap-3 max-w-sm">
                ...
                <div class="flex items-center justify-between gap-2">
                  <h3 class="font-bold text-base truncate">Unit #<?= htmlspecialchars($transient_id); ?></h3>
                  <span class="badge badge-sm <?= transientStatusBadge($status); ?>">
            ...
            <?php else: ?>
              <div class="text-center py-8 text-base-content/50">
                ...
                No transient unit registered under this space yet.
              </div>
            <?php endif; ?>
          </div>
