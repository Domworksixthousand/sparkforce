...
$rate = "";
$ps_id = $square_area = $type_ps = $status = "";
$apartment_images = [];
$saved_amenities = [];
...
if ($rent_id) {
    ...
    if ($result_ps->num_rows > 0) {
        ...
        $type_ps     = $row_ps['type']        ?? '';
        $status      = $row_ps['status']      ?? 'Available';
    }
...
function parkingStatusBadge($status) {
            ...
            <div class="relative h-64 sm:h-80 md:h-[360px] w-full rounded-xl overflow-hidden bg-base-300 group shadow-inner">
              ...
              <div class="absolute top-4 left-4">
                <span class="badge badge-success text-white badge-lg gap-2 shadow-md">
                  <i class="fa-solid fa-square-parking"></i> <?= htmlspecialchars(ucfirst($type_ps ?: 'Parking Space')); ?>
                </span>
              </div>

               <a href="report.php?user_id=<?php echo urlencode($user_id); ?>&location_back=ps_details.php&id=<?php echo urlencode($rent_id); ?>&report_type=Post" class="<?php echo $hidden; ?> tooltip tooltip-left absolute bottom-4 right-27 lg:top-4 h-fit text-blue-900 bg-green-300 hover:bg-blue-200 active:bg-red-400 rounded-lg p-1.5 flex items-center justify-center transition-all cursor-pointer border-none outline-none" data-tip="Report">
              ...
              <form action="../functions.php" method="POST" class="tooltip tooltip-left absolute bottom-4 right-4 h-fit lg:top-4" data-tip="<?= ($switch == 'on') ? 'Remove from Favorites' : 'Add to my Favorites'; ?>">
                ...
                <input type="hidden" name="locate" value="ps_details.php">
            ...
            <div>
              ...
              <div class="bg-primary/10 p-4 rounded-xl border border-primary/20">
                ...
                <div class="text-3xl font-black text-success mt-1">
                  ₱<?= number_format((float)$price, 2); ?> <span class="text-sm font-normal text-base-content/70">/ <?php echo htmlspecialchars($rate); ?></span>
                </div>
              </div>

              <div class="grid grid-cols-2 gap-3 mt-4">
                <div class="stat bg-base-200/60 p-3 rounded-xl">
                  <div class="stat-title text-xs">Floor Area</div>
                  <div class="stat-value text-xl"><?= htmlspecialchars($square_area !== '' ? $square_area : 'N/A'); ?><span class="text-sm font-normal"> sqm</span></div>
                </div>
                <div class="stat bg-base-200/60 p-3 rounded-xl">
                  <div class="stat-title text-xs">Status</div>
                  <div class="stat-value text-xl">
                    <span class="badge <?= parkingStatusBadge($status); ?>"><?= htmlspecialchars($status !== '' ? $status : 'Unknown'); ?></span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-300">
            <h2 class="text-lg font-bold mb-3 flex items-center gap-2">
              <i class="fa-solid fa-circle-info text-success"></i> About This Parking Space
            </h2>
            <p class="text-base-content/80 text-sm leading-relaxed whitespace-pre-line">
              <?= $other_info ? $other_info : 'No description available for this parking space.'; ?>
            </p>
          </div>
          ...
          <div class="bg-base-100 p-6 rounded-2xl shadow-sm border border-base-300">
            <div class="flex items-center justify-between mb-4">
              <h2 class="text-lg font-bold flex items-center gap-2">
                <i class="fa-solid fa-square-parking text-success"></i> Parking Unit Details
              </h2>
              <span class="text-xs text-base-content/60">Unit ID: <?= htmlspecialchars($ps_id ?: 'N/A'); ?></span>
            </div>

            <?php if (!empty($ps_id)): ?>
              <div class="border border-base-300 rounded-xl p-4 flex flex-col justify-between bg-base-100 hover:border-primary transition-all shadow-2xs gap-3 max-w-sm">

                <div class="flex items-center justify-between gap-2">
                  <h3 class="font-bold text-base truncate">Unit #<?= htmlspecialchars($ps_id); ?></h3>
                  <span class="badge badge-sm <?= parkingStatusBadge($status); ?>">
                    <?= htmlspecialchars($status !== '' ? $status : 'Available'); ?>
                  </span>
                </div>

                <div class="grid grid-cols-2 gap-2 text-xs text-base-content/80 bg-base-200/50 p-2.5 rounded-lg border border-base-200">
                  <?php if (!empty($type_ps)): ?>
                    <div class="flex items-center gap-1.5 col-span-2" title="Unit Type">
                      <i class="fa-solid fa-tag text-success text-[11px]"></i>
                      <span><strong>Type:</strong> <?= htmlspecialchars(ucfirst($type_ps)); ?></span>
                    </div>
                  <?php endif; ?>

                  <?php if (!empty($square_area)): ?>
                    <div class="flex items-center gap-1.5 col-span-2" title="Floor Area">
                      ...
                      <span><strong>Area:</strong> <?= htmlspecialchars($square_area); ?> sqm</span>
                    </div>
                  <?php endif; ?>
                </div>
                ...
                <div class="pt-1 border-t border-base-200 flex items-center justify-between">
                  ...
                  <p class="text-xs font-semibold text-success">
                    ₱<?= number_format((float)$price, 2); ?> <span class="font-normal text-base-content/60">/ <?= htmlspecialchars($rate); ?></span>
                  </p>
                </div>
            ...
            <?php else: ?>
              <div class="text-center py-8 text-base-content/50">
                <i class="fa-solid fa-square-parking text-3xl mb-2 block"></i>
                No parking unit registered under this space yet.
              </div>
            <?php endif; ?>
          </div>
