        ...
        <div class="space-y-4">
            ...
            <div class="space-y-1.5 pt-2">
                ...
                <div class="border border-slate-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-emerald-500/20 focus-within:border-emerald-500 transition-all">
                    <textarea
                        id="myEditor"
                        name="transient_other_info"><?php echo $other_info; ?></textarea>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative flex items-center">
                        ...
                        <select class="w-full pl-10 pr-3 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all" name="transient_type" required>
                            <option value=" <?php echo $type_transient; ?>" > <?php echo $type_transient; ?></option>
                            <option value="Studio Unit" <?php echo ($type == 'Studio Unit') ? 'selected' : ''; ?>>Studio Unit</option>
                            <option value="1-Bedroom Transient Unit" <?php echo ($type == '1-Bedroom Transient Unit') ? 'selected' : ''; ?>>1-Bedroom Transient Unit</option>
                            <option value="2-Bedroom Transient Unit" <?php echo ($type == '2-Bedroom Transient Unit') ? 'selected' : ''; ?>>2-Bedroom Transient Unit</option>
                            <option value="Entire House / Unit" <?php echo ($type == 'Entire House / Unit') ? 'selected' : ''; ?>>Entire House / Unit</option>
                            <option value="Private Room" <?php echo ($type == 'Private Room') ? 'selected' : ''; ?>>Private Room</option>
       ...
       <div class="space-y-4">
            ...
            <div id="amenities-container" class="space-y-2">
                ...
                if (isset($user_id_login) && !empty($user_id_login)) {
                    $stmt_all_amen = $conn->prepare("SELECT * FROM amenities WHERE user_id=? AND active=?");
                    $stmt_all_amen->bind_param("ss", $user_id_login, $active);
                } else {
                    $stmt_all_amen = $conn->prepare("SELECT * FROM amenities WHERE active=?");
                    $stmt_all_amen->bind_param("s", $active);
                }
                
                $stmt_all_amen->execute();
                $result_all_amen = $stmt_all_amen->get_result();

                $all_amenities = [];
                while ($row = $result_all_amen->fetch_assoc()) {
                    $all_amenities[] = $row;
                }

                $selected_ids = array_column($saved_amenities, 'amen_id');
                if (empty($selected_ids)) {
                    $selected_ids = [""];
                }

                $index = 0;
                foreach ($selected_ids as $selectedAmenId) {
                ?>
                    ...
                    <select class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                        ...
                        <?php foreach ($all_amenities as $amen) { ?>
                            <option value="<?php echo htmlspecialchars($amen['amen_id']); ?>"
                                <?php echo ($selectedAmenId == $amen['amen_id']) ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($amen['amenity']); ?>
                            </option>
                        <?php } ?>
                    </select>

                    <button type="button" class="remove-amen-btn p-2 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg text-xs transition-colors shrink-0 <?php echo ($index === 0) ? 'hidden' : ''; ?>" title="Remove">
...
tinymce.init({
    ...
    init_instance_callback: function (editor) {
        const content = <?php echo json_encode($transient_other_info); ?>;
        if (content) {
